...
وارد سایت Railway.app شوید.

روی New Project → Deploy from GitHub repo کلیک کنید.

ریپازیتوری Fork شده را انتخاب کنید.

Railway به‌صورت خودکار پروژه را Deploy می‌کند.
...
در صفحه داشبورد کلی، لینک VLESS پیش‌فرض (بدون محدودیت) را مشاهده و کپی کنید.

این لینک را در کلاینت دلخواه (v2rayNG، NekoBox، Streisand و...) وارد کنید.

برای ساخت لینک‌های جداگانه با محدودیت ترافیک، سرعت، آی‌پی و... به بخش مدیریت لینک‌ها بروید.
...
به بخش «گروه‌های ساب» در پنل (یا از طریق ربات تلگرام) بروید.

یک گروه جدید بسازید.

کانفیگ(های) مورد نظر را داخل همان گروه اضافه کنید.

لینک صفحه‌ی عمومی گروه را از همان‌جا کپی کنید.

🤖 ربات مدیریت تلگرام (اختیاری)
اگر می‌خواهید بدون باز کردن پنل وب، از طریق تلگرام کانفیگ‌ها را مدیریت کنید:

دو متغیر محیطی زیر را در تنظیمات سرویس Railway اضافه کنید:
...
بعد از ری‌استارت سرویس، به ربات پیام /start بدهید.
...
⚙️ متغیرهای محیطی
متغیر	توضیح	پیش‌فرض
ADMIN_PASSWORD	رمز عبور ورود به داشبورد وب	MATIXKING
...
RAILWAY_PUBLIC_DOMAIN	دامنه عمومی سرویس؛ به‌صورت خودکار توسط Railway تنظیم می‌شود
